...
LEAF(entrypoint)
    // entry here can be either cached or uncached
.set noreorder
.set noat
    // handle the timer interrupt first, saving the
    // lower 32 bits of k0 and k1
    ...
    // from this point on, k1 is safe to use
    ...
    // fallthrough into startup if entry is none of the above
.set at
.set reorder
END(entrypoint)
...
LEAF(__skException)
    // secure mode faults end up here
    // load the fault handler (uncached) and jump to it
    la      k0, __sk_exception_handler
    la      a0, __dummy_callback
...
.set noat
    ...
    // save registers according to the o32 ABI
    // this assumes SKCs come from a function call,
    // so only callee-saved registers are actually stored
    sd      AT, 0x18(sp)
.set at
    sd      s0, 0x90(sp)
    sd      s1, 0x98(sp)
    ...
    // no need to clear the cache for security (this is a homebrew SK),
    // but probably a good idea to match how retail SK handles it
    la      t2, icache_reset
...
LEAF(skc_handler)
    ...
    // fix eSKape
    bgeu    v0, t3, .bad_skc_num

    // but no point forbidding kernel calls in a jailbroken SK
    b .call_skc
...
1:
    ...
    la      t1, PHYS_TO_K1(MI_SK_EXCEPTION_REG)
    lw      t0, (t1)
    // deassert app entry and button trap entry bits
    and     t0, t0, ~((1 << 6) | (1 << 2))
    // prepare secure mode exit, but stay for now
    and     t0, t0, ~(1 << 0)
...
LEAF(__handle_timer_expiry_2)
    ...
    lw      k0, (k1)
    and     k0, k0, ~(1 << 3) // clear timer entry bit
